...
static void fifo_timer(struct timer_list *t)
{
    char ch;
    if (kfifo_is_empty(&mybuf) == 0)
    {
        kfifo_out(&mybuf, &ch, 1);
        printk(KERN_INFO "%s:timer removed char %c from mybuf\n", THIS_MODULE->name, (int)ch);
    ...
    {
        count = 0;
        printk(KERN_INFO "%s:timer stopped , mybuf is empty\n", THIS_MODULE->name);
        count = 0;
    }
}
...
static void start_timer(void)
{
    if (count == 0)
    {
        count = 1;
        mod_timer(&timer, jiffies + 1 * HZ);
        printk(KERN_INFO "%s:timer started\n", THIS_MODULE->name);
    }
    else
    {
        printk(KERN_INFO "%s:timer is already running\n", THIS_MODULE->name);
    }
}
...
static void stop_timer(void)
{
    if (count != 0)
    {
        ...
        count = 0;
        printk(KERN_INFO "%s:timer stop\n", THIS_MODULE->name);
    }
    else
    {
        printk(KERN_INFO "%s:timer is not running\n", THIS_MODULE->name);
    }
}
// device operations
...
static long pchar_ioctl(struct file *pfile, unsigned int cmd, unsigned long param)
{
    ...
    case FIFO_GETINFO:
        ...
        return 0;
    case FIFO_RESIZE:
        void *temp_array = kmalloc(kfifo_size(&mybuf), GFP_KERNEL);
        if (temp_array != 0)
        {
    ...
    int __init pchar_init(void)
    {
        ...
        timer_setup(&timer, fifo_timer, 0);
